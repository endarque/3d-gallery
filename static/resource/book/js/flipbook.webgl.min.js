...
FLIPBOOK.PageWebGL = function(t, e, i, o, s, n) {
    ...
    if (e.wrapperH && e.zoom) {
        ...
        this.disposed = !1;
		//debugger;
        var a = e.wrapperH * e.zoom < .8 * n.pageTextureSizeSmall ? n.pageTextureSizeSmall : n.pageTextureSize;
        "front" == t ? this.sizeFront == a ? i && i.call(this) : this.options.main.loadPage(this.indexF, a, function(t) {
            if (t) {
        ...
        }) : "back" == t && (this.sizeBack == a ? i && i.call(this) : this.options.main.loadPage(this.indexB, a, function(t) {
            if (t) {
    ...
    o.width = "100%", o.height = "750px", o.position = "relative", o.overflow = "hidden", this.options.cameraDistance = this.options.cameraDistance / 1.5
	...
	this.$pageR = jQuery(document.createElement("div")).addClass("R").css({
	...
	this.$pageRx = jQuery(document.createElement("div")).attr("id","yg_r")
	.addClass("R").css({
        width: 1e3 * this.options.pageWidth / this.options.pageHeight + "px",
        height: "1000px",
        position: "absolute",
	...
	this.$pageRInner = jQuery("<div>").css("pointer-events", "all").appendTo(this.$pageR), 
	this.$pageL = jQuery(document.createElement("div")).addClass("L").css({
        width: 1e3 * this.options.pageWidth / this.options.pageHeight + "px",
        height: "1000px",
        position: "absolute",
        top: "-500px",
	...
	this.$pageLx = jQuery(document.createElement("div")).attr("id","yg_l")
		.addClass("L").css({
        width: 1e3 * this.options.pageWidth / this.options.pageHeight + "px",
        height: "1000px",
        position: "absolute",
	...
	this.$pageLInner = jQuery(document.createElement("div")).css("pointer-events", "all").appendTo(this.$pageL), 
	this.$pageC = jQuery(document.createElement("div")).addClass("C").css({
	...
	this.$pageCx = jQuery(document.createElement("div")).attr("id","yg_c").addClass("C").css({
        width: 2e3 * this.options.pageWidth / this.options.pageHeight + "px",
        height: "1000px",
        position: "absolute",
        top: "-500px",
	...
	this.$pageCInner = jQuery(document.createElement("div")).css("pointer-events", "all").appendTo(this.$pageC), this.htmlLayer = new FLIPBOOK.CSS3DObject(t), this.Scene.add(this.htmlLayer), this.cssRenderer = new FLIPBOOK.CSS3DRenderer;
    this.wrapper;
    ...
    if (this.initialized) {
        if (t < 0 && (t = 0), t > this.options.pages.length && (t = this.options.pages.length), t % 2 != 0 && t--, t == this.rightIndex) return this.loadPages(), void this.turnPageComplete();
        this.goingToPage = t;
        o = this;
        ...
        this.rightIndex > t ? (i = 1 / (this.rightIndex - t) * this.options.pageFlipDuration * 1e3 / 6, 10 < this.rightIndex - t && (i = 0), this.rightIndex - 2 > t ? (this.prevPage(!1), 0 < i ? setTimeout(function() {
            ...
            o.loadPages(), o.turnPageComplete()
    ...
    1 == this.view ? 0 == this.flippedright ? this.focusLeft() : 0 == this.flippedleft ? this.focusRight() : this.isFocusedLeft() ? this.focusLeft() : this.focusRight() : 0 == this.rightIndex ? this.focusRight() : this.rightIndex >= this.options.numPages ? this.focusLeft() : this.focusBoth(), this.updateHtmlLayerPosition(), this.needsUpdate = !0
    ...
    for (var t = this.pages, e = this.options.main, i = 0; i < t.length; i++) {
        if ((h = t[i]).flippingLeft || h.flippingRight) return
    }
    this.options.cornerCurl && (0 == this.flippedleft ? this.pages[0].startCornerCurl() : this.pages[0].stopCornerCurl());
    var o = this.pages[this.flippedleft],
        s = this.pages[this.flippedleft - 1],
        n = (this.pages[this.flippedleft - 2], this.pages[this.flippedleft + 1], this.pages[this.flippedleft - 3], this.pages[this.flippedleft + 2], this.updateHtmlLayer),
        r = this.loadMorePages;
    for (i = 0; i < t.length; i++) {
        var h;
        (h = t[i]) !== o && h !== s || h.showMat(), s && h.index < s.index - 2 && (h.hideMat(), this.options.pdfMode || h.disposeMat()), o && h.index > o.index + 2 && (h.hideMat(), this.options.pdfMode || h.disposeMat())
    }
    o = this.pages[this.flippedleft], s = this.pages[this.flippedleft - 1], this.pages[this.flippedleft - 2], this.pages[this.flippedleft + 1], this.pages[this.flippedleft - 3], this.pages[this.flippedleft + 2], n = this.updateHtmlLayer, r = this.loadMorePages;
    var a = this;
    t = this.pages;
    (e = this.options.main).setLoadingProgress(.1), s ? s.load("back", function(t) {
        o ? o.load("front", function(t) {
            s.loadHTML("back", function() {
                o.loadHTML("front", function() {
                    n.call(a)
                })
            }), e.setLoadingProgress(1), r.call(a)
        }) : (s.loadHTML("back", function() {
            n.call(a)
        }), e.setLoadingProgress(1), r.call(a))
    }) : o.load("front", function(t) {
        o.loadHTML("front", function() {
            n.call(a)
        }), e.setLoadingProgress(1), r.call(a)
    })
    ...
    var i = this.options.pageWidth,
        o = (this.wrapperW, this.wrapperH, this.zoom, .5 * i * this.centerContainer.scale.x);
    this.moveToPos({
        x: o,
        y: 0
    }, t, e)
    ...
    this.options.pageHeight, this.options.pageWidth, this.wrapperW, this.wrapperH, this.zoom;
    this.moveToPos({
        x: 0,
        y: 0
    }, t, e)
    ...
    } else this.centerContainer.position.x = t.x, this.centerContainer.position.y = t.y
    ...
    if (!this.zooming) {
        ...
        if (newZoom = t > this.options.zoomMax ? this.options.zoomMax : t, newZoom = t < this.options.zoomMin ? this.options.zoomMin : t, newZoom == this.options.zoom) {
            ...
            x: s.x,
            y: s.y
        }, e).easing(FLIPBOOK.TWEEN.Easing.Sinusoidal.In).start()) : (this.zoom = newZoom, this.centerContainer.position.set(s.x, s.y, 0), d.updateHtmlLayerPosition(), this.updateCameraPosition(), this.zooming = !1), t <= 1 && t <= this.zoom && this.updateBookPosition(), this.options.main.onZoom(newZoom), this.loadPages()
    }
...
function(t) {
    ...
    function(t) {
        ...
        var p = t,
            ...
            m = 0,
            y = null,
            x = FLIPBOOK.TWEEN.Easing.Linear.None,
            M = FLIPBOOK.TWEEN.Interpolation.Linear,
            v = [],
            P = null,
            L = !1,
            O = null,
        ...
        this.to = function(t, e) {
            ...
            for (var e in FLIPBOOK.TWEEN.add(this), L = !(i = !0), y = void 0 !== t ? t : window.performance.now(), y += m, l) {
            ...
            var e;
            if (t < y) return !0;
            !1 === L && (null !== P && P.call(p), L = !0);
            var i = (t - y) / u,
                o = x(i = 1 < i ? 1 : i);
            for (e in l) {
                var s = c[e] || 0,
                    n = l[e];
                n instanceof Array ? p[e] = M(n, o) : ("string" == typeof n && (n = s + parseFloat(n, 10)), "number" == typeof n && (p[e] = s + (n - s) * o))
            }
            if (null !== O && O.call(p, o), 1 != i) return !0;
            if (0 < f) {
            ...
            null !== b && b.call(p);
            for (var h = 0, a = v.length; h < a; h++) v[h].start(t);
            return !1
        }
        ...
        function y(t) {
            for (var e, i = (e = [], t.traverse(function(t) {
                ...
                }), o = i.length, s = 0, n = i.length; s < n; s++) i[s].element.style.zIndex = o - s
        }
